    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            if(idMembre==0)
                ...
                message.initMessageEnvoye(idMembre);
                if(message.getId()==0)
                    request.setAttribute("info", 2);
                else {
                    if(request.getParameter("kermit")!=null) {
                        message.getPosts1(request);
                        message.verifPosts3(request);
                    }
                    request.setAttribute("message", message);
                }
            }
            RequestDispatcher dispatch = request.getRequestDispatcher("./scripts/messageenvoye.jsp");
            dispatch.forward(request, response);
            /* TODO output your page here
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet MessageEnvoye</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet MessageEnvoye at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
            */
        } finally { 
